...
def affine_matrix_from_points(v0, v1, shear=True, scale=True, usesvd=True):
  ...
  t0 = -jnp.mean(v0, axis=1)
  M0 = jnp.identity(ndims + 1)
  M0 = M0.at[:ndims, ndims].set(t0)
  #M0[:ndims, ndims] = t0
  v0 = v0 + t0.reshape(ndims, 1)
  t1 = -jnp.mean(v1, axis=1)
  M1 = jnp.identity(ndims + 1)
  M1 = M1.at[:ndims, ndims].set(t1)
  #M1[:ndims, ndims] = t1
  v1 = v1 + t1.reshape(ndims, 1)

  if shear:
    # Affine transformation
    A = jnp.concatenate((v0, v1), axis=0)
    u, s, vh = jnp.linalg.svd(A.T)
    vh = vh[:ndims].T
    B = vh[:ndims]
    C = vh[ndims:2 * ndims]
    t = jnp.dot(C, jnp.linalg.pinv(B))
    t = jnp.concatenate((t, jnp.zeros((ndims, 1))), axis=1)
    M = jnp.vstack((t, ((0.0,) * ndims) + (1.0,)))
    ...
    xx, yy, zz = jnp.sum(v0 * v1, axis=1)
    xy, yz, zx = jnp.sum(v0 * jnp.roll(v1, -1, axis=0), axis=1)
    xz, yx, zy = jnp.sum(v0 * jnp.roll(v1, -2, axis=0), axis=1)
  ...
  # move centroids back
  M = jnp.dot(jnp.linalg.inv(M1), jnp.dot(M, M0))
  M = M / M[ndims, ndims]
  return M
