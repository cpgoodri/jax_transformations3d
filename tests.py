...
class TransformationsTest(jtu.JaxTestCase):
  ...
  def test_identity_matrix(self):
    I = identity_matrix()
    self.assertAllClose(I, jnp.dot(I, I), True)
    self.assertAllClose(jnp.array([jnp.sum(I), jnp.trace(I)]), jnp.array([4.0, 4.0]), True)
    self.assertAllClose(I, jnp.identity(4), True)
  ...
  def test_reflection_matrix(self):
    ...
    self.assertAllClose(2, jnp.trace(R), False)
    self.assertAllClose(v0, jnp.dot(R, v0), True)
    ...
    v2 = jnp.array(v2)
    v3 = jnp.array(v3)
    self.assertAllClose(v2, jnp.dot(R, v3), True)
  ...
  def test_rotation_matrix(self):
    ...
    self.assertAllClose(jnp.dot(R, jnp.array([0, 0, 0, 1])), jnp.array([1, -1, 0, 1]), False)
    ...
    self.assertAllClose(2, jnp.trace(rotation_matrix(math.pi/2, direc, point)), False)
  ...
  def test_scale_matrix(self):
    ...
    self.assertAllClose(jnp.dot(S, v)[:3], -1.234*v[:3], True)
  ...
  def test_projection_matrix(self):
    ...
    self.assertAllClose(P[1:, 1:], jnp.identity(4)[1:, 1:], True)
    point  = random_vector(3) - 0.5
  ...
  def test_shear_matrix(self):
    ...
    self.assertAllClose(1.0, jnp.linalg.det(S), False, 1e-12, 1e-12)
  ...
  def test_euler_matrix(self):
    ...
    self.assertAllClose(jnp.sum(R[0]), -1.34786452, False)
    
    R = euler_matrix(1, 2, 3, (0, 1, 0, 1))
    self.assertAllClose(jnp.sum(R[0]), -0.383436184, False)
  ...
  def test_euler_from_quaternion(self):
    ...
    for axes in _AXES2TUPLE.keys():
      ...
      self.assertAllClose(jnp.array(R0[:3,:3]), jnp.array(R1), True)
  ...
  def test_quaternion_from_euler(self):
    ...
    for a,s in zip(axes_list, scipy_axes_list):
      ...
      #self.assertAllClose(q, jnp.array([0.435953, 0.310622, -0.718287, 0.444435]), True)
  ...
  def test_quaternion_matrix(self):
    ...
    M0 = quaternion_matrix(q)
    M1 = Rot.from_quat(_convert_quaternion(q)).as_matrix()
    self.assertAllClose(M0, jnp.identity(4), True)
    self.assertAllClose(M0[:3,:3], M1, False)
    
    q = jnp.array([0, 1, 0, 0])
    M0 = quaternion_matrix(q)
    M1 = Rot.from_quat(_convert_quaternion(q)).as_matrix()
    self.assertAllClose(M0, jnp.diag(jnp.array([1, -1, -1, 1], dtype=jnp.float64)), True)
    self.assertAllClose(M0[:3,:3], M1, False)
  ...
  def test_quaternion_from_matrix(self):
    ...
    self.assertAllClose(q, jnp.array([1, 0, 0, 0], dtype=jnp.float64), True)
    
    q = quaternion_from_matrix(jnp.identity(4), False)
    self.assertAllClose(q, jnp.array([1, 0, 0, 0], dtype=jnp.float64), True)

    q = quaternion_from_matrix(jnp.diag(jnp.array([1, -1, -1, 1],dtype=jnp.float64)), True)
    assert(jnp.allclose(q, jnp.array([0, 1, 0, 0], dtype=jnp.float64)) or jnp.allclose(q, jnp.array([0, -1, 0, 0], dtype=jnp.float64)))
    ...
    R = jnp.array([[-0.545, 0.797, 0.260, 0], [0.733, 0.603, -0.313, 0],
         [-0.407, 0.021, -0.913, 0], [0, 0, 0, 1]], dtype=jnp.float64)
    q0 = quaternion_from_matrix(R)
    #q1 = Rot.from_matrix(R[:3,:3]).as_quat()
    #self.assertAllClose(_convert_quaternion(q0), q1, True)
    self.assertAllClose(q0, jnp.array([0.19069, 0.43736, 0.87485, -0.083611]), True, atol=1e-5, rtol=1e-5)
    
    R = jnp.array([[0.395, 0.362, 0.843, 0], [-0.626, 0.796, -0.056, 0],
         [-0.677, -0.498, 0.529, 0], [0, 0, 0, 1]], dtype=jnp.float64)
    q0 = quaternion_from_matrix(R)
    #q1 = Rot.from_matrix(R[:3,:3]).as_quat()
    #self.assertAllClose(_convert_quaternion(q0), q1, True)
    self.assertAllClose(q0, jnp.array([0.82336615, -0.13610694, 0.46344705, -0.29792603]), True, atol=1e-5, rtol=1e-5)
  ...
  def test_quaternion_multiply(self):
    ...
    self.assertAllClose(q, jnp.array([28, -44, -14, 48], dtype=jnp.float64), True)
  ...
  def test_quaternion_inverse(self):
    ...
    self.assertAllClose(quaternion_multiply(q0, q1), jnp.array([1, 0, 0, 0], dtype=jnp.float64), True)
  ...
  def test_jit_quaternion_real(self):
    self.assertAllClose(jit(quaternion_real)(jnp.array([3, 0, 1, 2], dtype=jnp.float64)), 3.0, False)


  def test_quaternion_imag(self):
    self.assertAllClose(quaternion_imag([3, 0, 1, 2]), jnp.array([ 0.,  1.,  2.], dtype=jnp.float64), True)

  def test_jit_quaternion_imag(self):
    self.assertAllClose(jit(quaternion_imag)([3, 0, 1, 2]), jnp.array([ 0.,  1.,  2.], dtype=jnp.float64), True)
  ...
  def test_random_rotation_matrix(self):
    R = random_rotation_matrix()
    self.assertAllClose(jnp.dot(R.T, R), jnp.identity(4), True)
  ...
  def test_vector_norm(self):
    v = random_vector(3)
    n = vector_norm(v)
    self.assertAllClose(n, jnp.linalg.norm(v), True)
    
    v = jnp.array(onp.random.rand(6, 5, 3))
    n = vector_norm(v, axis=-1)
    self.assertAllClose(n, jnp.sqrt(jnp.sum(v * v, axis=2)), True)
    
    n = vector_norm(v, axis=1)
    self.assertAllClose(n, jnp.sqrt(jnp.sum(v * v, axis=1)), True)
  ...
  def test_unit_vector(self):
    v0 = random_vector(3)
    v1 = unit_vector(v0)
    self.assertAllClose(v1, v0 / jnp.linalg.norm(v0), True)
    self.assertAllClose(unit_vector([1]), jnp.array([1.0]), True)
  ...
  def test_vector_product(self):
    ...
    self.assertAllClose(v, jnp.array([0, 0, 6]), False)
    
    v0 = [[2, 0, 0, 2], [0, 2, 0, 2], [0, 0, 2, 2]]
    v1 = [[3], [0], [0]]
    v = vector_product(v0, v1)
    self.assertAllClose(v, jnp.array([[0, 0, 0, 0], [0, 0, 6, 6], [0, -6, 0, -6]]), False)

    v0 = [[2, 0, 0], [2, 0, 0], [0, 2, 0], [2, 0, 0]]
    v1 = [[0, 3, 0], [0, 0, 3], [0, 0, 3], [3, 3, 3]]
    v = vector_product(v0, v1, axis=1)
    self.assertAllClose(v, jnp.array([[0, 0, 6], [0, -6, 0], [6, 0, 0], [0, -6, 6]]), False)
  ...
  def angle_between_vectors(self):
    ...
    self.assertAllClose(a, jnp.array([0, 1.5708, 1.5708, 0.95532]), True)
    
    v0 = [[2, 0, 0], [2, 0, 0], [0, 2, 0], [2, 0, 0]]
    v1 = [[0, 3, 0], [0, 0, 3], [0, 0, 3], [3, 3, 3]]
    a = angle_between_vectors(v0, v1, axis=1)
    self.assertAllClose(a, jnp.array([1.5708, 1.5708, 1.5708, 0.95532]), True)
  

  def test_inverse_matrix(self):
    M0 = random_rotation_matrix()
    M1 = inverse_matrix(M0.T)
    self.assertAllClose(M1, jnp.linalg.inv(M0.T), True)
    
    for size in range(1, 7):
      M0 = jnp.array(onp.random.rand(size, size))
      M1 = inverse_matrix(M0)
      self.assertAllClose(M1, jnp.linalg.inv(M0), True)
  ...
  def test_concatenate_matrices(self):
    ...
    self.assertAllClose(jnp.dot(M, M.T), concatenate_matrices(M, M.T), True)
  ...
  def test_matrix_apply_and_quaternion_apply(self):
    ...
    v3  = random.uniform(split1, (3,), minval=-1.0, maxval=1.0, dtype=jnp.float64) 
    v4  = jnp.pad(v3, (0,1), mode='constant', constant_values=1.0)
    vm3 = random.uniform(split2, (10,3), minval=-1.0, maxval=1.0, dtype=jnp.float64) 
