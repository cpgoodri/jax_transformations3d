...
class TransformationsTest(jtu.JaxTestCase):
  ...
  def test_identity_matrix(self):
    ...
    self.assertAllClose(jnp.array([jnp.sum(I), jnp.trace(I)]), jnp.array([4.0, 4.0]))
  ...
  def test_rotation_matrix(self):
    ...
    self.assertAllClose(jnp.dot(R, jnp.array([0, 0, 0, 1])), jnp.array([1, -1, 0, 1]), check_dtypes=False)
    ...
    self.assertAllClose(2, jnp.trace(rotation_matrix(math.pi/2, direc, point)), check_dtypes=False)
  ...
  def test_jit_projection_matrix(self):
    ...
    P1 = jit(projection_matrix, static_argnums=(2,4))(point, normal, None, persp, True)
    self.assertAllClose(P0, P1)
  ...
  def test_shear_matrix(self):
    ...
    self.assertAllClose(1.0, jnp.linalg.det(S), check_dtypes=False, atol=1e-12, rtol=1e-12)
  ...
  def test_euler_from_quaternion(self):
    ...
    for axes in _AXES2TUPLE.keys():
      ...
      self.assertAllClose(jnp.array(R0[:3,:3]), jnp.array(R1), atol=1e-12, rtol=1e-12)
  ...
  def test_quaternion_matrix(self):
    ...
    M0 = quaternion_matrix(q)
    M1 = Rot.from_quat(_convert_quaternion(q)).as_matrix()
    self.assertAllClose(M0, jnp.diag(jnp.array([1, -1, -1, 1], dtype=jnp.float64)))
    self.assertAllClose(M0[:3,:3], M1, check_dtypes=False)
  ...
  def test_quaternion_from_matrix(self):
    ...
    q = quaternion_from_matrix(jnp.diag(jnp.array([1, -1, -1, 1],dtype=jnp.float64)))
    ...
    q1 = _check_scipy_quaternion_sign(Rot.from_matrix(onp.array(R[:3,:3])).as_quat())
    self.assertAllClose(_convert_quaternion(q0), q1)
    ...
    #self.assertAllClose(_convert_quaternion(q0), q1)
    self.assertAllClose(q0, jnp.array([0.19069, 0.43736, 0.87485, -0.083611]), atol=1e-5, rtol=1e-5)
    ...
    #q1 = Rot.from_matrix(R[:3,:3]).as_quat()
    #self.assertAllClose(_convert_quaternion(q0), q1)
    self.assertAllClose(q0, jnp.array([0.82336615, -0.13610694, 0.46344705, -0.29792603]), atol=1e-5, rtol=1e-5)
  ...
  def test_quaternion_inverse(self):
    ...
    self.assertAllClose(quaternion_multiply(q0, q1), jnp.array([1, 0, 0, 0], dtype=jnp.float64))
  ...
  def test_jit_quaternion_real(self):
    self.assertAllClose(jit(quaternion_real)(jnp.array([3, 0, 1, 2], dtype=jnp.float64)), 3.0, check_dtypes=False)


  def test_quaternion_imag(self):
    self.assertAllClose(quaternion_imag([3, 0, 1, 2]), jnp.array([ 0.,  1.,  2.], dtype=jnp.float64))

  def test_jit_quaternion_imag(self):
    self.assertAllClose(jit(quaternion_imag)([3, 0, 1, 2]), jnp.array([ 0.,  1.,  2.], dtype=jnp.float64))
  ...
  def test_vector_product(self):
    ...
    self.assertAllClose(v, jnp.array([[0, 0, 0, 0], [0, 0, 6, 6], [0, -6, 0, -6]]), check_dtypes=False)

    v0 = [[2, 0, 0], [2, 0, 0], [0, 2, 0], [2, 0, 0]]
    v1 = [[0, 3, 0], [0, 0, 3], [0, 0, 3], [3, 3, 3]]
    v = vector_product(v0, v1, axis=1)
    self.assertAllClose(v, jnp.array([[0, 0, 6], [0, -6, 0], [6, 0, 0], [0, -6, 6]]), check_dtypes=False)
